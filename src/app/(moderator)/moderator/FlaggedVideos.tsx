  ...
  if (videos.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] text-center px-4">
        ...
        <h2 className="font-heading text-xl font-bold text-warm-700 mb-2">All clear!</h2>
        <p className="text-sm text-warm-500">No flagged videos right now.</p>
      </div>
    )
  }
      ...
      {videos.map((video) => (
        <div key={video.id} className="card-warm space-y-4">
          <div className="flex gap-4 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-warm-400 mt-1">
                {video.flags.length} open {video.flags.length === 1 ? 'flag' : 'flags'} · status: {video.status}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold text-warm-600 uppercase tracking-wide">Flag reasons</p>
            {video.flags.map((flag) => (
              <div key={flag.id} className="bg-rose-50 border border-rose-200 rounded-xl px-3 py-2 text-sm text-rose-700">
                🚩 {flag.reason}
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              {pending[video.id] ? '…' : '✓ Clear flags & restore'}
            </button>
          </div>
        </div>
      ))}
