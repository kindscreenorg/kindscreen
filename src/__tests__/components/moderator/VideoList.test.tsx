...
describe('VideoList', () => {
  ...
  it('shows empty state when no videos match filter', async () => {
    ...
    render(<VideoList initialVideos={[makeVideo('1', { status: 'approved' })]} />)
  ...
  it('shows reject button on approved video', () => {
    render(<VideoList initialVideos={[makeVideo('1', { status: 'approved' })]} />)
    expect(screen.getByRole('button', { name: /remove from catalog/i })).toBeInTheDocument()
  })

  it('shows restore button on rejected video', () => {
    render(<VideoList initialVideos={[makeVideo('1', { status: 'rejected' })]} />)
    expect(screen.getByRole('button', { name: /restore/i })).toBeInTheDocument()
  })

  it('shows restore button on suspended video', () => {
    render(<VideoList initialVideos={[makeVideo('1', { status: 'suspended' })]} />)
    expect(screen.getByRole('button', { name: /restore/i })).toBeInTheDocument()
  })

  it('no action buttons for pending videos', () => {
    render(<VideoList initialVideos={[makeVideo('1', { status: 'pending' })]} />)
  ...
  it('shows error message on PATCH failure', async () => {
    mockFetch.mockResolvedValue({ ok: false, json: async () => ({ error: 'Not allowed' }) })
    const user = userEvent.setup()
    render(<VideoList initialVideos={[makeVideo('1', { status: 'approved' })]} />)
    await user.click(screen.getByRole('button', { name: /remove from catalog/i }))
    await waitFor(() => {
  ...
  it('shows thumbnail image when thumbnail_url is set', () => {
    render(<VideoList initialVideos={[makeVideo('1', { thumbnail_url: 'https://example.com/thumb.jpg' })]} />)
    expect(screen.getByAltText('Video 1')).toBeInTheDocument()
  })

  it('shows fallback emoji when thumbnail_url is null', () => {
    render(<VideoList initialVideos={[makeVideo('1', { thumbnail_url: null })]} />)
